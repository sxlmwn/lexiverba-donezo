...
export const TeamPage: React.FC<TeamPageProps> = ({ isDarkMode = false }) => {
  ...
  return (
    <div className="space-y-8">
      {/* Header matching team_lexiverba_bold */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <button className={`flex items-center gap-2 px-6 py-3.5 border font-black text-xs rounded-full shadow-2xs transition-all hover:scale-105 active:scale-95 ${
            isDarkMode ? 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700' : 'bg-blue-50/80 border-blue-200 text-blue-700 hover:bg-blue-100'
          }`}>
      ...
      {/* Main Grid: Left Capacity & Availability Widgets (4 Cols) + Right Member Cards Grid (8 Cols) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column Widgets - 4 Cols */}
        <div className="lg:col-span-4 space-y-8">
          {/* Active Capacity Widget */}
          <div className={`p-8 rounded-[2.5rem] border-2 shadow-sm transition-colors ${
            ...
            <div className="relative w-44 h-44 mx-auto my-6 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                <circle cx="88" cy="88" r="70" fill="transparent" stroke={isDarkMode ? '#334155' : '#e2e8f0'} strokeWidth="16" />
                <circle
                  cx="88"
                  cy="88"
                  r="70"
        ...
        {/* Right Member Cards Grid (Matching team_lexiverba_bold Aesthetic) - 8 Cols */}
        <div className="lg:col-span-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {teamMembers.map((member) => {
              ...
              return (
                <div
                  ...
                  className={`p-6 rounded-[2.5rem] border-2 transition-all duration-300 relative flex flex-col justify-between ${
                    isHovered
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-blue-600 font-extrabold mt-0.5 mb-3">{member.role}</p>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-700/80">
                    <div>
            ...
            {/* Add Member Card */}
            <div className={`p-6 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center text-center cursor-pointer transition-all hover:scale-105 ${
              isDarkMode ? 'border-slate-700 bg-slate-800/40 text-slate-300 hover:border-blue-500' : 'border-slate-300 bg-slate-50/60 text-slate-700 hover:border-blue-600'
            }`}>
      ...
      {/* Bottom Live Terminal Stream matching team_lexiverba_bold */}
      <div className={`p-6 rounded-[2.5rem] border-2 shadow-sm font-mono text-xs space-y-2 transition-colors ${
        isDarkMode ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-slate-900 border-slate-900 text-slate-200'
      }`}>
        ...
        <div className="space-y-1 font-semibold text-[11px] text-blue-300">
