  ...
  return (
    <header
      className={`fixed top-0 left-72 right-0 h-20 backdrop-blur-xl border-b-2 z-40 px-10 flex items-center justify-between transition-colors duration-300 ${
        isDarkMode ? 'bg-[#0e121e]/90 border-[#1e2638] text-slate-100' : 'bg-white/80 border-slate-200/50 text-slate-900'
      }`}
    >
      {/* Search Input */}
      <div className={`flex items-center rounded-2xl px-4 py-2.5 w-[420px] border transition-all ${
        isDarkMode ? 'bg-[#151c2c] border-[#222c42] focus-within:border-blue-500' : 'bg-slate-100/80 border-slate-200/60 focus-within:border-blue-600'
      }`}>
        <span className="material-symbols-outlined text-slate-400 text-[20px]">search</span>
        <input
          type="text"
          placeholder="Search task, projects, or documents..."
        ...
        <kbd className={`px-2 py-0.5 border text-[10px] font-black rounded-md shadow-2xs ${
          isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-white border-slate-200 text-slate-400'
        }`}>
          ⌘F
        </kbd>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all shadow-2xs hover:scale-105 active:scale-95 ${
            isDarkMode
              ? 'bg-[#182032] border-[#253046] text-amber-400 hover:bg-slate-800'
        ...
        <button className={`w-10 h-10 rounded-full border flex items-center justify-center transition-colors shadow-2xs relative ${
          isDarkMode ? 'bg-[#182032] border-[#253046] text-slate-300 hover:bg-slate-800' : 'bg-white border-slate-200/80 text-slate-600 hover:bg-slate-50'
        }`}>
